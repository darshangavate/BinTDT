...
import type { BatchData } from "../pages/Dashboard"; // adjust path if needed
...
export default function RewardsPanel({ batch }: RewardsPanelProps) {
  ...
  const societies = useMemo(
    () => [
      { name: "GreenHeights A", score: 94, waste: 52 },
      { name: "Harmony Park", score: 89, waste: 47 },
      { name: "Sunrise Co-op", score: 73, waste: 35 },
      { name: "EcoVille Homes", score: 68, waste: 29 },
    ],
    []
  );

  // Revenue & reward logic
  const batchRevenue = batch.totalWaste * 30; // assume ₹30/kg sale
  const rewardPool = batchRevenue * 0.15;     // 15% shared
  const totalScore = societies.reduce((a, s) => a + s.score, 0);
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-emerald-100 p-6">
      ...
      <div className="overflow-x-auto rounded-xl border border-emerald-100">
        <table className="min-w-full divide-y divide-emerald-100 text-sm">
          <thead className="bg-emerald-50">
            <tr>
              <th className="px-4 py-2 text-left font-semibold text-gray-700">Society</th>
              <th className="px-4 py-2 text-left font-semibold text-gray-700">Score</th>
              <th className="px-4 py-2 text-left font-semibold text-gray-700">Waste (kg)</th>
          ...
          <tbody className="divide-y divide-emerald-100">
            {rewards.map((s, i) => (
              <tr key={i} className="hover:bg-emerald-50/60 transition">
                ...
                <td className="px-4 py-2 font-semibold text-emerald-700">₹{s.reward}</td>
                <td className="px-4 py-2">
                  {s.score > 80 ? (
                    <span className="text-xs font-semibold text-emerald-700 bg-emerald-100 px-2 py-1 rounded-full">Released</span>
                  ) : (
                    <span className="text-xs font-semibold text-yellow-700 bg-yellow-100 px-2 py-1 rounded-full">Pending</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <div className="mt-4 text-right text-sm text-gray-600">
        Total Reward Pool : ₹{rewardPool.toFixed(0)} | Distributed among {societies.length} societies
      </div>
    </div>
  );
}
